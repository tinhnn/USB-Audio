...
const timer_cfg_t g_timer_dac_cfg =
		{ .mode = TIMER_MODE_PERIODIC,
				/* Actual period: 0.00002083 seconds. Actual duty: 49.9759961593855%. */.period_counts =
						(uint32_t) 0x823, .duty_cycle_counts = 0x411,
				.source_div = (timer_source_div_t) 0, .channel = 5,
				.p_callback = timer_dac_callback,
    ...
    .p_context           = NULL,
				...
				.p_context = &NULL,
#endif
				.p_extend = &g_timer_dac_extend, .cycle_end_ipl = (1),
...
const timer_instance_t g_timer_dac = { .p_ctrl = &g_timer_dac_ctrl, .p_cfg =
		&g_timer_dac_cfg, .p_api = &g_timer_on_gpt };
dac_instance_ctrl_t g_dac0_ctrl;
const dac_extended_cfg_t g_dac0_ext_cfg = { .enable_charge_pump = 0,
		.data_format = DAC_DATA_FORMAT_FLUSH_LEFT,
		.output_amplifier_enabled = 0, .internal_output_enabled = false,
		.ref_volt_sel = (0) };
const dac_cfg_t g_dac0_cfg = { .channel = 0, .ad_da_synchronized = false,
		.p_extend = &g_dac0_ext_cfg };
/* Instance structure to use this module. */
const dac_instance_t g_dac0 = { .p_ctrl = &g_dac0_ctrl, .p_cfg = &g_dac0_cfg,
		.p_api = &g_dac_on_dac };
...
const usb_cfg_t g_basic0_cfg = { .usb_mode = USB_MODE_PERI, .usb_speed =
		USB_SPEED_FS, .module_number = 0, .type = USB_CLASS_PAUD,
#if defined(NULL)
                .p_usb_reg = NULL,
#else
		.p_usb_reg = &NULL,
...
void usb_audio_thread_create(void) {
	...
	UINT err;
	err = tx_thread_create(&usb_audio_thread, (CHAR*) "USB Thread",
			usb_audio_thread_func, (ULONG) NULL, &usb_audio_thread_stack, 1024,
